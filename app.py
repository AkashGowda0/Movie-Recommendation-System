...
TMDB_API_KEY = '05a9d0b4c56a447a09eb76e6e99f3337'
...
def more_details(item_type, item_id):
    ...
    url = f"{TMDB_API_URL}/{item_type}/{item_id}?api_key={TMDB_API_KEY}&language=en-US&append_to_response=credits,images,reviews"
    response = requests.get(url)
    ...
    if response.status_code == 200:
        ...
        crew = details.get("credits", {}).get("crew", [])[:5]  # Get top 5 crew members

        # Fetch videos (e.g., trailer)
        ...
        # Fetch posters (limit to top 5)
        posters = details.get("images", {}).get("posters", [])[:5]

        # Fetch reviews (limit to top 5)
        reviews = details.get("reviews", {}).get("results", [])
        
        # Format the review dates properly
        for review in reviews:
            review["created_at"] = review["created_at"].split("T")[0]  # Extract only the date part
...
def underrated():
    ...
    params = {
        'api_key': TMDB_API_KEY,
        'language': 'en-US',
        'sort_by': 'popularity.asc',  # Underrated movies are less popular
        'page': 1,
        'vote_average.gte': 6,
        'vote_count.gte': 50,
    }
    ...
    return render_template('underrated.html', movies=movies, genres=genres, selected_genre=genre)
...
def recommend():
    ...
    # Mock mapping emotions to genres for example
    genre_map = {
        "happy": 35,  # Comedy
        "sad": 18,    # Drama
        "angry": 28,  # Action
